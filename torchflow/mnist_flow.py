...
class FMnistDTS(pl.LightningDataModule):
    ...
    def train_dataloader(self) -> DataLoader:
        d = FashionMNIST("/tmp/fmnist", download=True, transform=Compose([Resize(32), ToTensor()]), train=True)
        return DataLoader(d, batch_size=self.batch_size, num_workers=2, pin_memory=True)

    def val_dataloader(self) -> DataLoader:
        d = FashionMNIST("/tmp/fmnist", download=True, transform=Compose([Resize(32), ToTensor()]), train=False)
        return DataLoader(d, batch_size=self.batch_size, num_workers=2, pin_memory=True)
...
class MnistDTS(pl.LightningDataModule):
    ...
    def train_dataloader(self) -> DataLoader:
        d = MNIST("/tmp/mnist", download=True, transform=Compose([Resize(32), ToTensor()]), train=True)
        return DataLoader(d, batch_size=self.batch_size, num_workers=2, pin_memory=True)

    def val_dataloader(self) -> DataLoader:
        d = MNIST("/tmp/mnist", download=True, transform=Compose([Resize(32), ToTensor()]), train=False)
        return DataLoader(d, batch_size=self.batch_size, num_workers=2, pin_memory=True)
...
class CifarDTS(pl.LightningDataModule):
    ...
    def train_dataloader(self) -> DataLoader:
        d = CIFAR10("/tmp/cifar10", download=True, transform=Compose([Resize(32), ToTensor()]), train=True)
        return DataLoader(d, batch_size=self.batch_size, num_workers=5, pin_memory=True)

    def val_dataloader(self) -> DataLoader:
        d = CIFAR10("/tmp/cifar10", download=True, transform=Compose([Resize(32), ToTensor()]), train=False)
        return DataLoader(d, batch_size=self.batch_size, num_workers=5, pin_memory=True)
...
class CelebADTS(pl.LightningDataModule):
    ...
    def _split(self, dts):
        return random_split(dts, [self.train_size, self.val_size], generator=torch.Generator().manual_seed(11))

    def train_dataloader(self) -> DataLoader:
        dts = CelebA(self.root, download=True, transform=Compose([CenterCrop(178), Resize(64), ToTensor()]))
        return DataLoader(self._split(dts)[0], batch_size=self.batch_size, num_workers=2, pin_memory=True)

    def val_dataloader(self) -> DataLoader:
        dts = CelebA(self.root, download=True, transform=Compose([CenterCrop(178), Resize(64), ToTensor()]))
        return DataLoader(self._split(dts)[1], batch_size=self.batch_size, num_workers=2, pin_memory=True)
