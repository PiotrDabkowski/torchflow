...
def assert_close(actual, expected, abs_tolerance=1e-4, rel_tolerance=1e-3, reduction=torch.max):
    assert reduction(torch.abs(actual - expected)).item() <= abs_tolerance, (actual, expected)
    assert reduction(torch.abs(actual - expected) / (torch.abs(expected) + 1.)).item() <= rel_tolerance, (
    actual, expected)


def assert_same_flow(actual: Flow, expected: Flow, abs_tolerance=1e-3, rel_tolerance=1e-3, reduction=torch.max):
    ...
    def cmp(actual, expected):
        if actual is None or expected is None:
            ...
            assert_close(actual, expected, abs_tolerance, rel_tolerance, reduction=reduction)
        else:
            assert len(actual.shape) <= 1
            assert len(expected.shape) <= 1
            assert_close(actual.mean(), expected.mean(), abs_tolerance, rel_tolerance, reduction=reduction)
...
def test_encode_deterministic(module):
    ...
    assert_same_flow(module.encode(Flow(sample_tensor)), module.encode(Flow(sample_tensor)), abs_tolerance=1e-7,
                     rel_tolerance=1e-7, reduction=torch.max)
...
def test_decode(module):
    ...
    assert_same_flow(in_flow, decoded_flow, abs_tolerance=1e-4, rel_tolerance=1e-4, reduction=torch.max)
...
def test_logdet(module):
    ...
    logdet_estimate = torch.slogdet(calculate_jacobian(Flow(sample_tensor), module)).logabsdet
    ...
    if max_abs_logdet > 1:
        assert_close(logdet, logdet_estimate, abs_tolerance=5e-3 * max_abs_logdet, rel_tolerance=5e-3,
                     reduction=torch.max)
    else:
        assert_close(logdet, logdet_estimate, abs_tolerance=1e-3, rel_tolerance=0.5, reduction=torch.max)
...
def test_separation(split_fraction):
    ...
    assert_close(flow.logpz, sum_for(torch.log(normal_prob(flow.zs[0])), dim=0), abs_tolerance=1e-3)
...
def test_logpz_correct():
    ...
    assert flow.data is None, "Flow must be fully terminated at the end of the flow network."
    expected_logpz = torch.distributions.Normal(0.0, 1.0).log_prob(
        get_flow_flat_output(flow, sample_tensor.shape[0])).sum(1)
    assert_close(flow.logpz, expected_logpz, abs_tolerance=1e-2)
    data_bits = flow.get_elem_bits(sample_tensor.shape, 256)
    assert_close(data_bits, torch.zeros_like(data_bits) + 8, abs_tolerance=4, rel_tolerance=2)
